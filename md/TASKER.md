...
- `subject_template`: Configures the subject applied to the generated reply draft. It supports embedded dynamic date templates like `{yesterday}` and `{today}` which resolve to the `DD-Month` format. For example, `"Open TKTs {yesterday}"` will resolve to `"Open TKTs 01-September"`.
