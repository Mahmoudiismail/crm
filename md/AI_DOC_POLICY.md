...
## Recent Fixes
- **Pipeline Execution Engine:** Completely replaced the legacy execution model in `runner/engine.rs` with a deterministic pipeline execution engine processing `TaskStep` elements in `Sequential` or `Parallel` execution modes. External apps and shell commands are now natively managed within these steps.
