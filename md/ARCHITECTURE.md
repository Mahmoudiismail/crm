...
4.  **Yasweb Automation (`src/bin/yasweb.rs`)**: A headless Chrome automation utility used to log into an external Yasweb Angular dashboard, discover and configure filters, and extract generated data via iframe injection. It supports concurrent execution of monthly sliced reports via shared session tab isolation and CDP file download interception, enabling multiple reports to run simultaneously within a single browser instance by dynamically skipping redundant login processes. It also supports custom download paths via the `--download-path` CLI argument.
