...
#[test]
fn test_legacy_migration_and_serialization_roundtrip() {
    let legacy_json = r#"{
        "id": "legacy1",
        "name": "Legacy Task",
        "enabled": true,
    ...
    let task: RunnerTask = serde_json::from_str(legacy_json).expect("Deserialization failed");

    // Check step 0 action 0 (app_a) inherited legacy values
    if let ActionSpec::ExternalApp(ref a) = task.steps[0].actions[0] {
        assert_eq!(a.app_id, "app_a");
        assert_eq!(a.period_mode, PeriodMode::Monthly);
        assert_eq!(a.start_date.as_deref(), Some("2026-01-01"));
        assert_eq!(a.end_date.as_deref(), Some("2026-09-01"));
    } else {
        panic!("Expected ExternalApp for action 0");
    }

    // Check step 0 action 1 (app_b) kept explicit values
    if let ActionSpec::ExternalApp(ref b) = task.steps[0].actions[1] {
        assert_eq!(b.app_id, "app_b");
        assert_eq!(b.period_mode, PeriodMode::Quarterly);
        assert_eq!(b.start_date.as_deref(), Some("2026-02-01"));
        assert_eq!(b.end_date.as_deref(), Some("2026-12-01"));
    } else {
        panic!("Expected ExternalApp for action 1");
    }

    // Check post_run_steps action 0 (app_post) inherited legacy values
    if let ActionSpec::ExternalApp(ref post) = task.post_run_steps[0].actions[0] {
        assert_eq!(post.app_id, "app_post");
        assert_eq!(post.period_mode, PeriodMode::Monthly);
        assert_eq!(post.start_date.as_deref(), Some("2026-01-01"));
        assert_eq!(post.end_date.as_deref(), Some("2026-09-01"));
    } else {
        panic!("Expected ExternalApp for post run action 0");
    }

    // Test serialization round-trip: new serialized JSON must NOT contain task-level period_mode, start_date, end_date
    let reserialized = serde_json::to_string(&task).expect("Serialization failed");
    let json_val: serde_json::Value = serde_json::from_str(&reserialized).unwrap();
    let obj = json_val.as_object().unwrap();

    assert!(!obj.contains_key("period_mode"));
