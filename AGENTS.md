...
- Ensure Date filtering parsing for Excel Dashboard Updater outputs Month columns strictly as abbreviated names with full year formatting (`MMM-yyyy` like `Jan-2026`). Also ensure that standalone Day columns are never generated or exported to reports.
