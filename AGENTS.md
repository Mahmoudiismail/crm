...
- The tasker application resolves its download directory path relative to the executable directory and must use `std::time::SystemTime` for all file age and modification threshold comparisons across tasks (e.g., ticket data, leads data) to ensure reliable discovery without being affected by local timezone offsets or Daylight Saving Time shifts (e.g., discrepancies between Tasker execution time and PC time).
...
- In the tasker application, the dashboard_updater task triggers the generation of `results.csv` via csv_analysis, but it no longer natively parses, filters, or copy-pastes this data into the Excel dashboard. Instead, it relies on the Excel dashboard having a built-in PowerQuery/Data connection pointing to `results.csv`, and uses PowerShell COM automation to simply refresh the Data Model and all PivotTables within the file before emailing it out.
- When using COM automation to set `$Excel.Calculation = -4135` in PowerShell, ensure the operation is wrapped in a `try/catch` block to handle exceptions where the Excel object's state throws a `0x800A03EC` COM exception (which can happen when saving/closing an uninitialized or broken reference).
...
- Ensure Date filtering parsing for general CSV exports natively outputs Month columns strictly as abbreviated names with full year formatting (`MMM-yyyy` like `Jan-2026`). Also ensure that standalone Day columns are never generated or exported to reports.
- In `crm_open_sohail` when extracting Pivot Data dynamically using COM, explicitly call `$Pivot.RefreshTable()` before reading Slicer elements to ensure `DatasetData` properly registers numeric outputs. Wrap extracted Pivot row arrays in `@($DatasetData)` before checking `.Count` to avoid empty result parsing bugs.
- In `dashboard_updater`, always wrap `$Mail.Attachments.Add()` in a `try/catch` block to gracefully bypass server-side size limits without crashing the application.
- In `crm_open_sohail`, when dynamically building JSON strings for extracted Excel rows in PowerShell, the objects must be cast explicitly to `[PSCustomObject]@{...}` so that `ConvertTo-Json` natively serializes nested arrays properly.

- In Yasweb browser automation (`src/yasweb/browser.rs`), all injected JavaScript snippets executing within `tab.evaluate(..., true)` must explicitly `return await (async function() { ... })()`. Without the `return await` and returning the evaluation, the Promise is immediately resolved to `undefined` or dropped, causing `headless_chrome` to prematurely exit the evaluation without waiting for DOM elements. JavaScript step evaluations must also yield detailed JSON strings (e.g. `{"status": "SUCCESS", "logs": [...] }`) to provide trace debugging information back to the Rust host.

- When manually triggering an execution in the runner GUI, `is_manual: true` bypasses `due_now()` and `RunnerStatus::queued_task_ids` check in `run_task_by_id`, leaving duplicate queue logic entirely to the atomic execution manager (`ExecutionManagerCommand::QueueTask`) to prevent getting stuck in "Waiting".
- For `AppManifest` mapped dates in the runner GUI, setting `start_date_arg` and `end_date_arg` on `ExternalAppSpec` automatically populates the dynamic argument replacements at execution time without unconditionally injecting generic `--start-date`/`--end-date` args when overriden by the user.
- To generate full Live Preview boundaries accurately for empty schedules (e.g. `PeriodMode::Custom` with no `end_date`), the engine artificially bounds generation loops dynamically (`effective_period_end_utc`) up to 5 years rather than prematurely halting due to missing evaluation ranges.

- When using `SpecialCells(12)` in PowerShell COM automation, ensure to check boundaries (e.g. `$lastRow -ge $startRow`) or wrap the call in a `try/catch` block to handle empty datasets without throwing terminal COM exceptions.
- Ensure PowerShell subprocesses initiated via Rust (`std::process::Command` in script_manager.rs) are bounded by timeouts, strict max output buffer limits (e.g., 10MB), and utilize `taskkill /F /T /PID` to prevent zombie execution.
- When deploying persistent tasker COM automation on Windows, ensure robust COM cleanup is handled in `finally` blocks (using `Quit()` and `ReleaseComObject` with try/catch wrapping).