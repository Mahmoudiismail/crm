...
async fn initiate_auth(
    ...
    let initiate_url = if region.starts_with("http") {
        region.to_string()
    } else {
    ...
    if !status.is_success() {
        ...
        bail!("InitiateAuth failed (HTTP {}): {}", status, safe_msg);
    }
...
async fn respond_to_auth_challenge(
    ...
    let challenge_url = if region.starts_with("http") {
        region.to_string()
    } else {
        format!("https://cognito-idp.{}.amazonaws.com/", region)
    };
...
mod tests {
    ...
    fn test_compute_s_large_numbers() {
        ...
        let k = BigUint::parse_bytes(
            b"94781652145770828568693154820068118481071319461203609910235169525938894292833",
            10,
        )
        .unwrap();
        let x = BigUint::parse_bytes(
            b"57342845408344078087409546890185131247557055518447571803285707850707119990624",
            10,
        )
        .unwrap();
        ...
        let u = BigUint::parse_bytes(
            b"44444098212040351355717692468363509183647834247620902213903532544853357575578",
            10,
        )
        .unwrap();
